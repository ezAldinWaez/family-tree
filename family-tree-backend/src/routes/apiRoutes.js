  ...
  try {
      ...
      const transformedPerson = {
          ...
          relationships: person.relationships.map(rel => ({
              ...
              spouse: getBasicPersonInfo(
                  person.sex === 'male' ? rel.wife : rel.husb
              ),
...
router.post('/spouse', ValidateSuposeData, async (req, res) => {
  ...
  try {
      const { personId, spouse: spouseData, relationship: relationshipData } = req.body;
...
router.post('/child', validateChildData, async (req, res) => {
  ...
  try {
      ...
      if (childData.birth?.date) {
          ...
          if (relationship.husb.isDead && 
              relationship.husb.death?.date && 
              new Date(relationship.husb.death.date) < yearAfterBirthDate) {
              return res.status(400).json({
                  success: false,
                  error: 'Father was deceased before child\'s birth date for longer than a year'
              });
          }

          // Check mother's death date
          if (relationship.wife.isDead && 
  ...
  try {
      ...
      const canDeletePerson = async (person) => {
        ...
        if (relationshipsWithChildren.length > 0) {
            return {
                canDelete: false,
                reason: 'Cannot delete person who is a parent. This would break children\'s lineage.'
            };
        }
      ...
      for (const rel of spouseRelationships) {
          ...
          deletionSummary.relationships.push({
              ...
              otherSpouse: {
                  ...
                  fullName: rel.husb._id.equals(person._id) ? rel.wife.fullName : rel.husb.fullName
              }
          });
  ...
  try {
      ...
      if (updateData.birth?.date) {
          ...
          if (person.origin) {
              ...
              if (parentRelationship?.marriageStartDate && 
                  new Date(updateData.birth.date) < new Date(parentRelationship.marriageStartDate)) {
                  return res.status(400).json({
                      success: false,
          ...
          const hasChildrenAfterBirth = person.relationships.some(rel => 
              rel.children.some(child => 
                  child.birth?.date && new Date(child.birth.date) < new Date(updateData.birth.date)
              )
          );
...
router.put('/relationship/:relationshipId', async (req, res) => {
  ...
  try {
      ...
      if (!validateMarriageDates(updateData.marriageStartDate, updateData.marriageEndDate)) {
          return res.status(400).json({
              success: false,
      ...
      if (updateData.marriageStartDate && relationship.children.length > 0) {
          const hasChildrenBeforeMarriage = relationship.children.some(child =>
              child.birth?.date && new Date(child.birth.date) < new Date(updateData.marriageStartDate)
          );

          if (hasChildrenBeforeMarriage) {
              return res.status(400).json({
                  success: false,
                  error: 'Marriage start date cannot be after existing children\'s birth dates'
              });
          }
      }
      ...
      if (updateData.state) {
          ...
          if (relationship.state !== updateData.state && 
          ...
          if (updateData.state === 'widowed') {
              ...
              if (!husbDead && !wifeDead) {
                  return res.status(400).json({
                      success: false,
                      error: 'Cannot set relationship to widowed when both spouses are alive'
                  });
              }
          }
      }
